(function (e) {
  ...
  var c = {},
    ...
    b = '<style id="css-ddslick" type="text/css">.dd-desc { color:#aaa; display:block; overflow: hidden; font-weight:normal; line-height: 1.4em; }.dd-options > li:last-child > .dd-option{ border-bottom:none;}.dd-selected-description-truncated { text-overflow: ellipsis; white-space:nowrap; }.dd-image-right { float:right; margin-right:15px; margin-left:5px;}.dd-container{ position:relative;}​ .dd-selected-text { font-weight:bold}​</style>';
  c.init = function (l) {
    var l = e.extend({}, d, l);
...
window.addEventListener('DOMContentLoaded', () => { // Ждём загрузки DOM дерева 
  ...
  const addBackground = () => {
    const up = document.querySelector('.dd-pointer');
    const sele = document.querySelector('.dd-select');
    const selected = document.querySelector('.dd-selected');
    const option = document.querySelector('.dd-options');
  
    sele.addEventListener('click', () => {
      if(up.closest('.dd-pointer-up')) {
        selected.classList.add('__act')
      } else {
        selected.classList.remove('__act')
      }
    })
    ...
    body.addEventListener('click', (e) => {
      let target = e.target;
      if(!target.closest('.dd-container')) {
        selected.classList.remove('__act')
      } 
    })
  }
